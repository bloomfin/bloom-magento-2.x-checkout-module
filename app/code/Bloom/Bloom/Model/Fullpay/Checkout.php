<?php
namespace Bloom\Bloom\Model\Fullpay;
...
class Checkout
{
    ...
    /**
     * Gets the BloomPayments fulfill from the admin config
     * @return string
     */
    ...
    /**
     * Gets the BloomPayments secret key from the admin config
    ...
    public function start()
    {
        ...
        if (!$this->_quote->getGrandTotal() && !$this->_quote->hasNominalItems()) {
            throw new \Exception('Bloom does not support processing orders with zero amount.');
        }
        ...
        $request['ecommerce_module'] = "Bloom Magento Module 0.1.0";
        $request['checkout_settings'] = array("extended_cart" => 'true');
        $request['hooks'] = array(
	    "user_return_url_on_success" => $this->coreUrl->getUrl('bloom/onepage/success'),
	    "user_return_url_on_fail"=>$this->coreUrl->getUrl('bloom/onepage/failure'),
	    "terms_url"=> $this->coreUrl->getUrl('terms'),
	    "webhook_url"=> $this->coreUrl->getUrl('bloom/confirmations/index')
        );
    ...
    {
        return $this->checkoutSession->getBloomToken();
    }
